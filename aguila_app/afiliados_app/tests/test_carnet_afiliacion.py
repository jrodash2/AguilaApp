...
class CarnetAfiliacionTests(TestCase):
    ...
    def test_download_script_exports_real_jpeg_at_high_resolution(self):
        ...
        self.assertIn("document.getElementById('qrValidacion')", script)
        self.assertIn('window.html2canvas(previewShell', script)
        self.assertIn('var carnetRect = previewShell.getBoundingClientRect()', script)
        self.assertIn('var qrRect = qrImage.getBoundingClientRect()', script)
        self.assertIn('previewShell.contains(qrImage)', script)
        self.assertIn('qrImage instanceof HTMLImageElement', script)
        self.assertIn("qrImage.style.visibility = 'hidden'", script)
        self.assertIn("backgroundColor: '#ffffff'", script)
        self.assertIn('exportContext.drawImage(', script)
        self.assertIn('downloadButton.disabled = true', script)
        self.assertIn('downloadButton.disabled = false', script)
        self.assertNotIn('esperarImagenes', script)
        ...
        response = self.client.get(reverse('afiliados:carnet_afiliado', args=[self.afiliado.pk]))
        self.assertContains(response, 'id="qrValidacion"')
