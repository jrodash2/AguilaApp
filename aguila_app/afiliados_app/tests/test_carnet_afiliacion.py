...
class CarnetAfiliacionTests(TestCase):
    ...
    def test_download_script_exports_real_jpeg_at_high_resolution(self):
        ...
        self.assertIn('var qrRect = qrLink.getBoundingClientRect()', script)
        self.assertIn('previewShell.contains(qrImage)', script)
        self.assertIn('loadImage(qrImage.currentSrc || qrImage.src)', script)
        self.assertIn('exportContext.drawImage(', script)
        self.assertIn('validarQrDibujado(exportContext, qrX, qrY, qrWidth, qrHeight)', script)
