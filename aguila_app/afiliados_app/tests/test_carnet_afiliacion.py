...
class CarnetAfiliacionTests(TestCase):
    ...
    def test_download_script_exports_real_jpeg_at_high_resolution(self):
        ...
        self.assertIn("context.fillText('CÓDIGO DE AFILIACIÓN', px(159), px(505))", script)
        self.assertIn("context.fillText(data.codigo, px(490), px(505))", script)
        self.assertIn("document.getElementById('qr-validacion-img')", script)
        self.assertIn('window.html2canvas(previewShell', script)
        self.assertIn('previewShell.contains(qrImg)', script)
        self.assertIn('qrImg instanceof HTMLImageElement', script)
        self.assertIn("backgroundColor: '#ffffff'", script)
        self.assertIn('esperarImagen(qrImg)', script)
        self.assertIn('renderCard(assets[0], assets[1], assets[2])', script)
        self.assertIn('drawContain(qrImg, px(713), px(18), px(125), px(125))', script)
        self.assertIn('downloadButton.disabled = true', script)
        self.assertIn('downloadButton.disabled = false', script)
        self.assertIn('esperarImagenes(previewShell)', script)
        self.assertNotIn("qrImg.style.visibility = 'hidden'", script)
        self.assertNotIn('getImageData(', script)
        self.assertNotIn('No fue posible verificar el QR en el carnet exportado.', script)
        self.assertIn("exportCanvas.toDataURL('image/jpeg', 0.95)", script)
        ...
        response = self.client.get(reverse('afiliados:carnet_afiliado', args=[self.afiliado.pk]))
        self.assertContains(response, 'id="qr-validacion-img"')
        self.assertContains(response, 'html2canvas/1.4.1/html2canvas.min.js')
